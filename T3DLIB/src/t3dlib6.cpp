...
T3DLIB_API void (* Draw_Object4D_Gouraud_ZBufferRW)(OBJECT4DV1 * pobj, CAM4DV1 * pcam, MATERIALV1 * pmaterial) = NULL;
...
T3DLIB_API void Draw_Object4D_Gouraud_ZBufferRW16(OBJECT4DV1 * pobj, CAM4DV1 * pcam, MATERIALV1 * pmaterial)
{
	...
	memcpy(&rc._SURFACE, pcam->psurf, sizeof(rc._SURFACE));
	memcpy(&rc._ZBUFFER, pcam->pzbuf, sizeof(rc._ZBUFFER));
	pmaterial;//memcpy(&rc._TEXTURE, &pmaterial->texture, sizeof(rc._TEXTURE));
