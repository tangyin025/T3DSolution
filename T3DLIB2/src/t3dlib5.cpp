...
T3DLIB_API void (* Draw_Rectangle)(register RENDERCONTEXTV1 * prc, const VERTEXV1 * pv0, const VERTEXV1 * pv1) = NULL;
T3DLIB_API void (* Draw_Rectangle_Alpha)(register RENDERCONTEXTV1 * prc, const VERTEXV1 * pv0, const VERTEXV1 * pv1) = NULL;
T3DLIB_API void (* Draw_Clipped_Rectangle)(register RENDERCONTEXTV1 * prc, const VERTEXV1 * pv0, const VERTEXV1 * pv1) = NULL;
T3DLIB_API void (* Draw_Clipped_Rectangle_Alpha)(register RENDERCONTEXTV1 * prc, const VERTEXV1 * pv0, const VERTEXV1 * pv1) = NULL;
T3DLIB_API void (* Draw_HLine)(register RENDERCONTEXTV1 * prc, const VERTEXV1 * pv0, const VERTEXV1 * pv1) = NULL;
T3DLIB_API void (* Draw_VLine)(register RENDERCONTEXTV1 * prc, const VERTEXV1 * pv0, const VERTEXV1 * pv1) = NULL;
T3DLIB_API void (* Draw_Clipped_HLine)(register RENDERCONTEXTV1 * prc, const VERTEXV1 * pv0, const VERTEXV1 * pv1) = NULL;
T3DLIB_API void (* Draw_Clipped_VLine)(register RENDERCONTEXTV1 * prc, const VERTEXV1 * pv0, const VERTEXV1 * pv1) = NULL;
T3DLIB_API void (* Draw_Line)(register RENDERCONTEXTV1 * prc, const VERTEXV1 * pv0, const VERTEXV1 * pv1) = NULL;
T3DLIB_API void (* Draw_Clipped_Line)(register RENDERCONTEXTV1 * prc, const VERTEXV1 * pv0, const VERTEXV1 * pv1) = NULL;
...
T3DLIB_API bool Clip_Line(register RENDERCONTEXTV1 * prc, VERTEXV1 * pv0, VERTEXV1 * pv1)
{
	...
	{
	case CLIP_REGION_N:
		cx0 = pv0->x + (pv1->x - pv0->x) * (prc->fmin_clip_y - pv0->y) / (pv1->y - pv0->y);
		cy0 = prc->fmin_clip_y;
		break;

	case CLIP_REGION_S:
		cx0 = pv0->x + (pv1->x - pv0->x) * (prc->fmax_clip_y - pv0->y) / (pv1->y - pv0->y);
		cy0 = prc->fmax_clip_y;
		break;

	case CLIP_REGION_W:
		cx0 = prc->fmin_clip_x;
		cy0 = pv0->y + (prc->fmin_clip_x - pv0->x) * (pv1->y - pv0->y) / (pv1->x - pv0->x);
		break;

	case CLIP_REGION_E:
		cx0 = prc->fmax_clip_x;
		cy0 = pv0->y + (prc->fmax_clip_x - pv0->x) * (pv1->y - pv0->y) / (pv1->x - pv0->x);
		break;
	...
	{
	case CLIP_REGION_N:
		cx1 = pv1->x + (pv0->x - pv1->x) * (prc->fmin_clip_y - pv1->y) / (pv0->y - pv1->y);
		cy1 = prc->fmin_clip_y;
		break;

	case CLIP_REGION_S:
		cx1 = pv1->x + (pv0->x - pv1->x) * (prc->fmax_clip_y - pv1->y) / (pv0->y - pv1->y);
		cy1 = prc->fmax_clip_y;
		break;

	case CLIP_REGION_W:
		cx1 = prc->fmin_clip_x;
		cy1 = pv1->y + (prc->fmin_clip_x - pv1->x) * (pv0->y - pv1->y) / (pv0->x - pv1->x);
		break;

	case CLIP_REGION_E:
		cx1 = prc->fmax_clip_x;
		cy1 = pv1->y + (prc->fmax_clip_x - pv1->x) * (pv0->y - pv1->y) / (pv0->x - pv1->x);
		break;
...
T3DLIB_API void Draw_Rectangle_Alpha16(register RENDERCONTEXTV1 * prc, const VERTEXV1 * pv0, const VERTEXV1 * pv1)
{
	...
	while(dy--)
	{
		...
		while(c--)
		{
			...
			//		COLOR_MUL_16R(((XRGB16 *)&prc->c_src_alpha)->r, ((XRGB16 *)&pv0->c_diff)->r),
			//		COLOR_MUL_16R(((XRGB16 *)&prc->c_dst_alpha)->r, ((XRGB16 *)s)->r));
			//((XRGB16 *)s)->g = COLOR_ADD_16G(
			//		COLOR_MUL_16G(((XRGB16 *)&prc->c_src_alpha)->g, ((XRGB16 *)&pv0->c_diff)->g),
			//		COLOR_MUL_16G(((XRGB16 *)&prc->c_dst_alpha)->g, ((XRGB16 *)s)->g));
			//((XRGB16 *)s)->b = COLOR_ADD_16B(
			//		COLOR_MUL_16B(((XRGB16 *)&prc->c_src_alpha)->b, ((XRGB16 *)&pv0->c_diff)->b),
			//		COLOR_MUL_16B(((XRGB16 *)&prc->c_dst_alpha)->b, ((XRGB16 *)s)->b));

			*(unsigned short *)s = (unsigned short)_RGB16BIT(
					COLOR_ADD_16R(
						COLOR_MUL_16R(_16BIT_GETR((unsigned short)prc->c_src_alpha), _16BIT_GETR((unsigned short)pv0->c_diff)),
						COLOR_MUL_16R(_16BIT_GETR((unsigned short)prc->c_dst_alpha), _16BIT_GETR(*(unsigned short *)s))),
					COLOR_ADD_16G(
						COLOR_MUL_16G(_16BIT_GETG((unsigned short)prc->c_src_alpha), _16BIT_GETG((unsigned short)pv0->c_diff)),
						COLOR_MUL_16G(_16BIT_GETG((unsigned short)prc->c_dst_alpha), _16BIT_GETG(*(unsigned short *)s))),
					COLOR_ADD_16B(
						COLOR_MUL_16B(_16BIT_GETB((unsigned short)prc->c_src_alpha), _16BIT_GETB((unsigned short)pv0->c_diff)),
...
T3DLIB_API void Draw_Rectangle_Alpha32(register RENDERCONTEXTV1 * prc, const VERTEXV1 * pv0, const VERTEXV1 * pv1)
{
	...
	while(dy--)
	{
		...
		while(c--)
		{
			...
			//		COLOR_MUL_32R(((XRGB32 *)&prc->c_src_alpha)->r, ((XRGB32 *)&pv0->c_diff)->r),
			//		COLOR_MUL_32R(((XRGB32 *)&prc->c_dst_alpha)->r, ((XRGB32 *)s)->r));
			//((XRGB32 *)s)->g = COLOR_ADD_32G(
			//		COLOR_MUL_32G(((XRGB32 *)&prc->c_src_alpha)->g, ((XRGB32 *)&pv0->c_diff)->g),
			//		COLOR_MUL_32G(((XRGB32 *)&prc->c_dst_alpha)->g, ((XRGB32 *)s)->g));
			//((XRGB32 *)s)->b = COLOR_ADD_32B(
			//		COLOR_MUL_32B(((XRGB32 *)&prc->c_src_alpha)->b, ((XRGB32 *)&pv0->c_diff)->b),
			//		COLOR_MUL_32B(((XRGB32 *)&prc->c_dst_alpha)->b, ((XRGB32 *)s)->b));

			*(unsigned int *)s = (unsigned int)_RGB32BIT(
					COLOR_ADD_32R(
						COLOR_MUL_32R(_32BIT_GETR((unsigned int)prc->c_src_alpha), _32BIT_GETR((unsigned int)pv0->c_diff)),
						COLOR_MUL_32R(_32BIT_GETR((unsigned int)prc->c_dst_alpha), _32BIT_GETR(*(unsigned int *)s))),
					COLOR_ADD_32G(
						COLOR_MUL_32G(_32BIT_GETG((unsigned int)prc->c_src_alpha), _32BIT_GETG((unsigned int)pv0->c_diff)),
						COLOR_MUL_32G(_32BIT_GETG((unsigned int)prc->c_dst_alpha), _32BIT_GETG(*(unsigned int *)s))),
					COLOR_ADD_32B(
						COLOR_MUL_32B(_32BIT_GETB((unsigned int)prc->c_src_alpha), _32BIT_GETB((unsigned int)pv0->c_diff)),
